...
import { AppContextCategorie,  AppContextCourielDepartToday, AppContextDestinateur, AppContextToken } from "./context/contextApp";
...
function CourielDepartDelete({rowId}){
	...
	const handleDeleteCourielDepart = (e) => {
		...
		deleteCourielDepartById(id)
			.then(resp =>{
				 const newCat = stateCourielDepartToday.filter((u) => u.id != id);
				 setStateCourielDepartToday(newCat);
				 alert(resp.data);
    ...
    return(
        ...
        <Modal.Header closeButton>
            <Modal.Title>Supprimer un courriel de départ</Modal.Title>
...
export default CourielDepartDelete;