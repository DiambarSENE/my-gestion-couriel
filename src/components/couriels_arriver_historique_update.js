...
import { getAllCourrielsArriverNotToday, getCourielArriverById,  updateCourielArriver } from "./servicesBackEnd/gestion-couriel-api";
import { AppContextCategorie, AppContextCourielArriverNotToday, AppContextExpediteur } from "./context/contextApp";
...
function CourielArriverHistoriqueUpdate({rowId}){
		...
		const errorDateDarriver = ValidationDateDarriver(dateDarriver);
		 ...
		 if(!errorExpediteur && !errorDateCorrespondance && !errorNumeroCorrespondance && !errorObjet && !errorDateDarriver){
			...
			updateCourielArriver(fd,{
                ...
                alert("Couriel modifié avec succès");
               {handleClose();}
                getAllCourrielsArriverNotToday().then(response => {
		...
		<a type="button" className="btn btn-sm btn-primary" onClick={handleShow}><i className="la la-pencil"></i></a>
	     <Modal show={show} onHide={handleClose}>
				<Modal.Header closeButton>
					<Modal.Title>Modifier Couriel arrive</Modal.Title>
				...
				<Modal.Body>
				  <form onSubmit={handlerUpdateCourielArriver}>
					...
					<div className="form-group">
						...
						<input min={0} value={numeroCorrespondance} type="number" onChange={(e) => setNumeroCorrespondance(e.target.value)} defaultValue={0}  className="form-control " placeholder="Entrer le Numero de la Correspondance"  />
					</div>
					</div>
...
export default CourielArriverHistoriqueUpdate;